...
| 🔧⁹ | `scripts/patch-wwebjs-expose-race.js`                      | whatsapp-web.js Puppeteer utility    | Backports upstream PR #201861. Post-QR navigation can start concurrent `inject()` pipelines; both can observe a page binding as absent before either registers it, so the slower `page.exposeFunction()` rejects with `window[name] already exists` and leaves the post-auth bridge partially attached. The patch treats only that exact already-present race as success; every other Puppeteer error still propagates.                                                                                                                                                                                                                                                                                                                                  | exact-shape match; remove once upstream makes concurrent binding registration idempotent.                                                                                                                                                                                                                                                                             |
...
| 🔧⁹ binding registration    | No separate row mark — keeps the **wwjs** post-QR `initialize` pipeline from losing a concurrent page-binding registration race. The `initialize` row already carries 🔧⁴ for the other readiness repair.                                                                                                                                              |
